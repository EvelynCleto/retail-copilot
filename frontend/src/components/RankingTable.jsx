...
function isPct(k,v)  {
  // detecta % pelo nome da coluna OU pelo valor numérico < 100 (mesmo se inteiro como 13)
...
function fmtCell(v, money, count, pct) {
  ...
  if (!isNaN(n)) {
    ...
    // percentual: pelo nome da coluna garante 13.0% correto
...
export default function RankingTable({ data, insight }) {
  ...
  const firstNum = valCols.find(c => isNum(data,c));
  const maxVal   = firstNum ? Math.max(...data.map(r=>Math.abs(Number(r[firstNum])||0))) : 0;
        ...
        <table style={t.table}>
          <thead>
            <tr style={t.headRow}>
              ...
              {firstNum && !series && <th style={{...t.th,width:68}}/>}
            </tr>
          </thead>
          <tbody>
            {data.map((row,i)=>{
              const top  = i<3 && !series;
              const pct  = firstNum && maxVal>0 ? (Math.abs(Number(row[firstNum])||0)/maxVal)*100 : 0;
                ...
                <tr key={i} className="rt-row" style={i%2===0?t.even:t.odd}>
                  {!series&&(
                    <td style={{...t.td,textAlign:"center",paddingRight:0}}>
                      {top ? <span style={{fontSize:15}}>{MDL[i]}</span>
                           : <span style={t.rank}>{i+1}</span>}
                    </td>
                  )}
                  <td style={{...t.td,fontWeight:top?600:400,color:top?"var(--text-primary)":"#374151"}}>{lv}</td>
                  {valCols.map(c=>(
                    <td key={c} style={{
                      ...t.td,textAlign:"right",
                      fontFamily:(isMoney(c)||isCount(c))?"'JetBrains Mono',monospace":"inherit",
                      fontSize:12, color:top?"var(--text-primary)":"#374151",
                    }}>
                  ...
                  {firstNum&&!series&&(
                    <td style={t.barTd}>
                      <div style={t.barBg}>
                        <div style={{...t.barFill,width:`${pct.toFixed(1)}%`,
                          background:top?"var(--linx)":"var(--text-muted)",
                          opacity:top?0.75:0.3}}/>
                      </div>
                    </td>
                  )}
                </tr>
            ...
            {series && Object.keys(totals).length>0 && (
              <tr style={t.totalRow}>
                <td style={{...t.td,...t.totalTd,fontWeight:700,color:"var(--text-primary)"}}>Total</td>
                {valCols.map(c=>(
...
const t = {
  ...
  th:      { padding:"9px 14px", background:"#F8FAFC",
  ...
  barTd:   { padding:"8px 12px 8px 4px", width:66 },
  ...
  insight: { padding:"9px 14px 10px", fontSize:12, color:"var(--text-secondary)",
